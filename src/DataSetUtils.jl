export LoadCamvid, LoadCamvidFull, LoadBearVideo, GetTVvalues
...
function LoadBearVideo(basepath::String,nr_output_chan::Int)
  ...
  #allocate tensors for data/label storage
  video  = zeros(Float32,6,480,854,77,1)
  ...
  for i=1:length(all_video_files)
      ...
      video[1:3,:,:,counter,1] = Images.channelview(temp_img)
  ...
  #repeat data to increase channel count explicitly for input-output
  repeat_factor = Int(nr_output_chan/3)
  video    = repeat(train_data,outer=[1,1,1,repeat_factor,1])
  ...
  for i=1:length(all_label_files)
      ...
      println(size(Images.channelview(temp_img)))
      mask[:,:,counter,1,1] = Images.channelview(temp_img)
      global counter = counter+1
      println(counter)
   end
   ...
   mask  = mask[:,1:424,1:72,:,:]
    ...
    #create a vector with one label per entry
    data   = Vector{Any}(undef,1)
    data_V = Vector{Any}(undef,1)
    labels   = Vector{Any}(undef,1)
    ...
    return data, data_V, labels, labels_V
end #end function
